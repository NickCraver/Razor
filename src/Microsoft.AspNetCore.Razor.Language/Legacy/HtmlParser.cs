...
namespace Microsoft.AspNetCore.Razor.Language.Legacy
{
    internal class HtmlParser : TokenizerBackedParser<HtmlTokenizer>
    {
        ...
        private void ParseMarkupNodes(in SyntaxListBuilder<RazorSyntaxNode> builder, Func<SyntaxToken, bool> stopCondition = null)
        {
            stopCondition = stopCondition ?? (token => EndOfFile);
            while (!stopCondition(CurrentToken))
            {
                ParseMarkupNode(builder);
            }
        }

        private void ParseMarkupNode(in SyntaxListBuilder<RazorSyntaxNode> builder)
        {
        ...
        private void ParseJavascript(in SyntaxListBuilder<RazorSyntaxNode> builder)
        {
            // TODO
        }
        ...
        private RazorSyntaxNode ParseMarkupComment()
        {
            using (var pooledResult = Pool.Allocate<RazorSyntaxNode>())
            {
                ...
                while (!EndOfFile)
                {
                    ParseMarkupNodes(htmlCommentBuilder, t => t.Kind == SyntaxKind.DoubleHyphen);
        ...
        private bool AcceptTokenUntilAll(in SyntaxListBuilder<RazorSyntaxNode> builder, params SyntaxKind[] endSequence)
        {
            while (!EndOfFile)
            {
                ParseMarkupNodes(builder, t => t.Kind == endSequence[0]);
        ...
        private ParserState GetParserState()
        {
            ...
            var whitespace = EatWhile(IsSpacingToken(includeNewLines: true));
            try
            {
